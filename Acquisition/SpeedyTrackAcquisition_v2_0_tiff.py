...
# -- functions to control GUI interactions --
    ...
    def settingfilename(self):
        global filename, filenametif, filenameinf, fileframes, filenametime
        filename=str(self.setpath.text())+str(self.setfilebase.text())+str(self.setiteration.text())+".fits"
        filenametif=str(self.setpath.text())+str(self.setfilebase.text())+str(self.setiteration.text())+".tif"
...
#--- functions to control data display ---
    ...
    def updateData(self, n): #Speedytrack data display and saving
        global img, data, frame1, filenametif, writing, file2
        writing=1
        self.FKview.setImage(data)
        self.FKview.setCurrentIndex(n)
        writing=1
        #file2=open(filenamedax, 'ab')
        tf.imwrite(filenametif, frame1, photometric='minisblack', append=True)
        #file2.close
        writing=0
...
#--- functions to run data acquisition ---
    def runFKacquisition(self, progress_callback): #SpeedyTrack acquisition 
        global numberframes, cam, data, shiftheight, numshift, filename, offset, filenametif, filenametime, fileframes, writing, livestatus, exttrig, frame1, file2
        ...
        for currentframe in range(numberframes):
            if livestatus==True:
                ...
                tstart1=time.time()
                while cam.acquisition_in_progress()==True:
                    if livestatus==False:
...
##        file2.close
    ...
    def runLiveView(self, progress_callback):
        ...
        if savestatus==1:
            ...
            filenametifimag=str(self.setpath.text())+"image_"+str(self.setfilebase.text())+str(self.setiteration.text())+".tif"
        ...
        if savestatus==1:
            ...
            file3.write(str3)
            file3.close
            tf.imwrite(filenametifimag, data, photometric='minisblack', append=True)
